...
def create_select_etudiant_nouveau(
    ...
    etudiant_in.id = uuid.id4()
    mention = crud.mention.get_by_id(db=db, uuid=etudiant_in.id_mention)
    if not mention:
        raise HTTPException(status_code=400, detail=f" Mention not found.", )
    all_etutiant = crud.nouveau_etudiant.get_by_mention(schema, etudiant_in.id_mention)
...
def update_etudiant(
    ...
    mention = crud.mention.get_by_id(db=db, uuid=etudiant.id_mention)
    if not mention:
        raise HTTPException(status_code=400, detail=f" Mention not found.", )

    journey = crud.journey.get_by_id(db=db, uuid=etudiant_in.id_journey)
    if not journey:
    ...
    etudiant_in.id_mention = mention.id
...
def read_etudiant_by_mention(
    ...
    if not anne_univ:
        raise HTTPException(status_code=400, detail=f"{decode_schemas(schema=schema)} not found.", )
    etudiant = crud.nouveau_etudiant.get_by_mention(schema=schema, id_mention=id_mention)
...
def read_etudiant_by_mention(
    ...
    if not anne_univ:
        raise HTTPException(status_code=400, detail=f"{decode_schemas(schema=schema)} not found.", )
    etudiant = crud.nouveau_etudiant.get_by_journey(schema=schema, id_journey=id_journey)
    return etudiant
