...
def create_table_note(
    ...
    if crud.user.is_superuser(current_user):
        ...
        list_semester = crud.journey_semester.get_by_journey(db=db, id_journey=journey.id)
        semester_list = []
        for semester_ in list_semester:
            semester_list.append(semester_.semester)
        ...
        for session in sessions:
            test_note = crud.note.check_table_exist(semester=semester, journey=journey.abbreviation,
                                                    session=session)
            if not test_note:
                models.create_table_note(journey=journey.abbreviation, semester=semester,
                ...
                if len(column_) != 0:
                    models.update_table_note(journey=journey.abbreviation, semester=semester,
...
def get_all_columns(
    ...
    interaction_value = jsonable_encoder(interaction)
    list_value = []
    if not interaction or not interaction_value[semester.lower()]:
        raise HTTPException(
            status_code=400,
...
def inserts_student(
    ...
    if session.lower() == "rattrapage":
        ...
        students = crud.ancien_student.get_by_class(db=db, id_mention=id_mention,
                                                    id_journey=id_journey, semester=semester)
...
def get_all_notes(
    ...
    if num_carte != "" and num_carte is not None and num_carte != "null":
        ...
        all_note_count = crud.note.read_all_note_count(semester=semester, journey=journey.abbreviation,
                                                       session=session, year=college_year, value_ue=value_ue,
                                                       value_ec=value_ec,
                                                       type_=type_, credit=credit, mean=mean, value=value)
    ...
    for note in all_notes_ue.ue:
        for column_ in create_model(columns):
            if note.name == column_['name']:
                ...
                et_un_rattrapage = crud.note.read_by_num_carte(semester=semester, journey=journey.abbreviation,
                                                               session='Rattrapage', num_carte=all_notes_ue.num_carte)
                if et_un:
                    ue_in = {}
                    ue_in_final = {}
                    note_ue = 0
                    ...
                    for i, ec in enumerate(column_['ec']):
                        ...
                        if value_sess is None:
                            ...
                            if et_un_rattrapage is not None:
                                value_rattrapage = et_un_rattrapage[f"ec_{note.ec[i].name}"]
                                value_sess = value_rattrapage
                            else:
                                value_sess = value_ec_note
...
def details_note(
    ...
    if student:
        ...
        stud.journey = crud.journey.get(db=db, id=student.id_journey)
        mention = crud.mention.get(db=db, id=stud.journey.id_mention)
