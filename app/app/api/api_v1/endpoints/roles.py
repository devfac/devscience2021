...
def read_roles(
    ...
    count = crud.role.get_count(db=db)
...
def update_role(
    ...
    Update an role.
    """
    role = crud.role.get_by_id(db=db, uuid=uuid)
    if not role:
...
@router.get("/by_id/", response_model=schemas.Role)
def read_role(
        *,
        db: Session = Depends(deps.get_db),
        uuid: str,
...
def delete_role(
    ...
    if not crud.user.is_superuser(current_user):
        raise HTTPException(status_code=400, detail="Not enough permissions")
    role = crud.role.remove(db=db, id=id_role)
    return role
