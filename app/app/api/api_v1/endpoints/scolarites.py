...
def certificat(
    ...
    if student:
        journey = crud.journey.get_by_id(db=db, uuid=student.id_journey).title
        mention = crud.mention.get_by_id(db=db, uuid=student.id_mention).title
        data = {'last_name': student.last_name, 'first_name': student.first_name, 'date_birth': student.date_birth,
                ...
                'journey': journey,
                'register': creaate_registre(student.enter_years)
                }
        date_ = date.today()
        file = create_certificat_scolarite(num_carte, date_.year, year, data)
...
def attestation(
    ...
    if student:
        ...
        data = {'last_name': student.last_name, 'first_name': student.first_name, 'date_birth': student.date_birth,
                'place_birth': student.place_birth,
                'level': get_niveau(student.inf_semester, student.sup_semester), 'mention': mention,
                'journey': journey, 'register': creaate_registre(student.enter_years)}
        date_ = date.today()
        anne = student.actual_years
        file = attestation_inscription(num_carte, date_.year, year, data)
        return FileResponse(path=file, media_type='application/octet-stream', filename=file)
    else:
        raise HTTPException(status_code=404, detail="Etudiant not found")
...
def relever(
    ...
    if et_un_final and student:
        ...
        interaction = crud.interaction.get_by_journey_and_year(
            db=db, id_journey=journey.id, college_year=college_year)
