...
def read_ancien_student(
    ...
    """
    students = crud.ancien_student.get_by_mention(db=db, order_by=order_by, order=order, id_journey=id_journey,
                                                  semester=semester, id_mention=id_mention, num_carte=num_carte,
                                                  limit=limit, skip=offset, id_year=id_year)
    all_student = []
    count = 0
    for student in crud.ancien_student.count_by_mention(db=db, id_mention=id_mention, id_journey=id_journey,
                                                        semester=semester, num_carte=num_carte):
        if find_in_list(current_user.id_mention, str(student.id_mention)) != -1 and student.id_journey:
            count += 1
    for on_student in students:
        print(on_student.actual_years, on_student.num_carte)
        if on_student.id_journey:
            for receipt in on_student.receipt_list:
                receipt = receipt.replace("'", '"')
                receipt = json.loads(receipt)
                if receipt['year'] == id_year:
                    on_student.receipt = receipt
                    break
            stud = schemas.AncienStudent(**jsonable_encoder(on_student))
            stud.journey = crud.journey.get_by_id(db=db, uuid=on_student.id_journey)
            if find_in_list(current_user.id_mention, str(stud.id_mention)) != -1:
...
def read_new_student(
    ...
    """
    students = crud.new_student.get_by_mention(db=db, id_mention=id_mention,
                                               college_year=college_year, limit=limit, skip=offset)
    all_student = []
    count = len(crud.new_student.count_by_mention(db=db, id_mention=id_mention, college_year=college_year))
    for on_student in students:
        on_student.mention = crud.mention.get_by_id(db=db, uuid=on_student.id_mention)
        if find_in_list(current_user.id_mention, str(on_student.id_mention)) != -1 and \
...
def read_new_student(
    ...
    Retrieve new student.
    """
    students = crud.new_student.get_by_mention(db=db, id_mention=id_mention, college_year=college_year, level=level,
                                               order_by=order_by, order=order, limit=limit, skip=offset)
    all_student = []
    count = 0
    for student in crud.new_student.count_by_mention(db=db, id_mention=id_mention, college_year=college_year,
                                                     level=level):
        if find_in_list(current_user.id_mention, str(student.id_mention)) != -1 and \
                find_in_list(student.actual_years, college_year) != -1 and student.id_journey:
            count += 1
    for on_student in students:
        if on_student.id_journey:
            for receipt in on_student.receipt_list:
            ...
            stud.journey = crud.journey.get_by_id(db=db, uuid=on_student.id_journey)
            stud.mention = crud.mention.get_by_id(db=db, uuid=on_student.id_mention)
            if find_in_list(current_user.id_mention, str(on_student.id_mention)) != -1 and \
...
def create_ancien_student(
    ...
    Create new student.
    """
    journey = crud.journey.get(db=db, id=student_in.id_journey)
    if not journey:
        raise HTTPException(status_code=404, detail="Journey not found")
    mention = crud.mention.get(db=db, id=student_in.id_mention)
    if not mention:
    ...
    if not student:
        ...
        if student:
            receipt_in = schemas.Receipt(**jsonable_encoder(student_in.receipt), num_carte=student_in.num_carte)
            crud.receipt.create(db=db, obj_in=receipt_in)
            crud.student_years.create(db=db, obj_in={"num_carte": student_in.num_carte, "id_year": id_year})
...
def create_new_student(
    ...
    for student_in in students:
        student_in.id_mention = id_mention
        student_in.id_journey = id_journey
...
def create_new_student(
    ...
    mention = crud.mention.get_by_id(db=db, uuid=student_in.id_mention)
    if not mention:
        raise HTTPException(status_code=404, detail="Mention not found")
...
def update_student_selected(
    ...
    journey = crud.journey.get_by_id(db=db, uuid=student_in.id_journey)
    if not journey:
        raise HTTPException(status_code=404, detail="Journey not found")
...
def read_student_by_num_carte(
    ...
    if not student:
        raise HTTPException(status_code=404, detail="Student not found")
    if student.id_journey:
        stud = schemas.AncienStudent(**jsonable_encoder(student))
        stud.journey = crud.journey.get_by_id(db=db, uuid=student.id_journey)
        for receipt in student.receipt_list:
            receipt = receipt.replace("'", '"')
            receipt = json.loads(receipt)
            if receipt['year'] == college_year:
...
def read_student_by_num_select(
    ...
    """
    student = crud.new_student.get_by_num_select(db=db, num_select=num_select)
    if not student or not find_in_list(current_user.id_mention, str(student.id_mention)) != -1:
        raise HTTPException(status_code=404, detail="Student not found")
...
def read_student_by_journey(
    ...
    """
    student = crud.ancien_student.get_by_jouney(db=db, id_journey=id_journey,
                                                college_year=college_year, limit=limit, skip=offset)
    return student
...
def read_student_by_journey(
    ...
    Get student by journey.
    """
    student = crud.new_student.get_by_jouney(db=db, id_journey=id_journey,
                                             college_year=college_year, limit=limit, skip=offset)
    return student
...
def read_student_by_sup_semester_and_mention(
    ...
    students = crud.ancien_student.get_by_sup_semester_and_mention(
        db=db, id_mention=id_mention, sup_semester=sup_semester,
        college_year=college_year, limit=limit, skip=offset)
    all_student = []
    for on_student in students:
        if on_student.id_journey:
