...
def get_list_user(db: Session = Depends(deps.get_db),*, client_id: str='') -> List[str]:
    ...
    if user:
        ...
        for on_user in all_users:
            if user.id_mention and on_user.email != user.email:
                for id_mention in user.id_mention:
                    if (find_in_list(on_user.id_mention, id_mention) != -1 and on_user.is_admin) or \
