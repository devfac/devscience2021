...
def all_statistic_(
    ...
    all_journey = crud.journey.get_by_mention(db=db, id_mention=id_mention)
    ...
    for index, level in enumerate(all_level):
        for journey in all_journey:
            if level[level_[index]][0] in journey.semester or level[level_[index]][1] in journey.semester:
                info = {"name": journey.abbreviation.upper(), "uuid": journey.id}
    ...
    for index, niveau in enumerate(all_niveau):
        for journey in all_journey:
            if niveau[niveau_[index]][0] in journey.semester or niveau[niveau_[index]][1] in journey.semester:
                ...
                etudiants = crud.ancien_student.get_by_journey_for_stat(college_year=college_year, id_journey=str(journey.id),
...
def statistic_by_nation(
    ...
    for index, niveau in enumerate(all_niveau):
        etudiants = crud.ancien_student.get_by_mention_and_niveau(college_year, str(id_mention), niveau[niveau_[index]])
...
def statistic_by_diplome(
    ...
    etudiants_num = crud.diploma.get_by_mention(schemas, str(id_mention))
    etudiants = []
...
def renseignement(
    ...
    if not mention:
        raise HTTPException(status_code=400, detail=f" Mention not found.", )
    niveau = ["L1", "M1", "M2"]
    etudiants = crud.nouveau_etudiant.get_by_mention(schemas, str(id_mention))
    info = {}
    for niv in niveau:
        all_etudiants = []
    ...
    file = stat_renseignement.create_stat_renseignement(data, info, db, id_mention)
    return FileResponse(path=file, media_type='application/octet-stream', filename=file)
...
def bachelier_(
    ...
    if not mention:
        raise HTTPException(status_code=400, detail=f" Mention not found.", )
    niveau = ["L1"]
    etudiants = crud.new_student.get_by_mention(id_mention)
    info = {}
    for niv in niveau:
        all_etudiants = []
        for un_etudiant in etudiants:
...
def dashboard(
    ...
    mention_list:List[str]= current_user.id_mention
    for id_mention in mention_list:
        mention = crud.mention.get_by_id(db=db, uuid=id_mention)
        if mention:
            students = crud.ancien_student.get_by_mention(db=db, id_mention=id_mention, skip=0,
