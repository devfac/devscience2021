...
from app.schemas.matier import ConstituentElementUpdate, ConstituentElementCreate, ConstituentElement


class CRUDNote(CRUDBase[ConstituentElement, ConstituentElementCreate, ConstituentElementUpdate]):
    ...
    def search_notes(self, semester: str,
        ...
        conn = engine.connect()
        sel = table.select()
        sel = sel.where(and_(table.columns["year"] == year, table.columns["num_carte"].like("%"+num_carte+"%")))
        sel = sel.order_by(table.columns.num_carte.asc())
        sel = sel.offset(skip)
        sel = sel.limit(limit)
        result = conn.execute(sel)
...
note = CRUDNote(ConstituentElement)
