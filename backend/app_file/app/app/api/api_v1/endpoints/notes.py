    ...
    if crud.user.is_superuser(current_user):
        ...
        for session in sesions:
            test_note = crud.note.check_table_exist(schemas=schemas, semestre=semestre, parcours=parcours.abreviation,
                                                    session=session)
            if not test_note:
                app.models.note.create_table_note(schemas=schemas, parcours=parcours.abreviation, semestre=semestre,
                ...
                if len(matier) != 0:
                    app.models.note.update_table_note(schemas=schemas, parcours=parcours.abreviation, semestre=semestre,
                                                      matiers=matiers, session=session)
                else:
...
def delete_table_note(
    ...
    if crud.user.is_superuser(current_user):
        ...
        test_note = crud.note.check_table_exist(schemas=schemas, semestre=semestre, parcours=parcours.abreviation,
                                                session=session)
        if test_note:
            if app.models.note.drop_table_note(schemas=schemas, parcours=parcours.abreviation, session=session,
                                               semestre=semestre):
                return {"msg": "Succces"}
            else:
                raise HTTPException(
