...
class CRUDEtudiantAncien(CRUDBase[EtudiantAncien, EtudiantAncienCreate, EtudiantAncienUpdate]):
    ...
    def get_by_parcours_and_etat_and_moyenne(self, schema: str, uuid_parcours: str, etat:str, moyenne:float) -> Optional[EtudiantAncien]:
        metadata = MetaData(schema=schema, bind=engine)
        table = Table("ancien_etudiant", metadata,autoload=True)
        conn = engine.connect()
        sel = table.select()
        sel = sel.where(and_(table.columns.uuid_parcours == uuid_parcours,table.columns.etat == etat,table.columns.moyenne >= moyenne))
        sel = sel.order_by(table.columns.nom.asc())
        result = conn.execute(sel)
        out = result.fetchall()
        conn.close()
    ...
    def get_by_parcours_and_etat_and_niveau(self, schema: str, uuid_parcours: str, etat:str, sems_petit:str, sems_grand:str) -> Optional[EtudiantAncien]:
        ...
        sel = table.select()
        sel = sel.where(and_(table.columns.uuid_parcours == uuid_parcours,table.columns.etat == etat,
            or_(table.columns.semestre_grand == sems_petit.upper(),table.columns.semestre_grand == sems_grand.upper())))
        sel = sel.order_by(table.columns.nom.asc())
        result = conn.execute(sel)
        out = result.fetchall()
        conn.close()
    ...
    def get_by_parcour_and_etat_and_moyenne_and_niveau(self, schema: str,uuid_parcours :str, etat: str, moyenne: float, sems_petit:str, sems_grand:str) -> Optional[EtudiantAncien]:
        metadata = MetaData(schema=schema, bind=engine)
        table = Table("ancien_etudiant", metadata,autoload=True)
        conn = engine.connect()
        sel = table.select()
