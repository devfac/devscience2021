...
@router.get("/new/all", response_model=List[schemas.NewStudent])
def read_new_student(
    ...
    """
    students = crud.new_student.get_all(db=db, college_year=college_year)
    return students
...
@router.post("/new", response_model=List[schemas.NewStudent])
def create_new_student(
        *,
        db: Session = Depends(deps.get_db),
        student_in: schemas.SelectStudentBase,
        current_user: models.User = Depends(deps.get_current_active_user),
) -> Any:
    """
    Create new student.
    ...
    student_in.num_select = f"S{mention.abbreviation.upper()}{student_in.num_select}{student_in.enter_year[2:4]}"
    student_in.plugged = mention.plugged
...
def update_student_selected(
    ...
    student_in.plugged = mention.plugged
    students = crud.new_student.get_student_admis(db=db, plugged=mention.plugged,
                                                  college_year=student_in.actual_years)
    all_students = []
