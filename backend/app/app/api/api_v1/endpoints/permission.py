...
def create_permission(
    *,
    ...
    permission = crud.permission.get_by_email_and_type(db=db, email=permission_in.email, type_=permission_in.type)
    if not permission:
        permission = crud.permission.create(db=db, obj_in=permission_in, email_sender=current_user.email)
    else:
        permissionUpdate: schemas.InvitationUpdate(**jsonable_encoder(permission_in), email_sender=current_user.email,
                                                   updated_at=datetime.datetime.now())
        permission = crud.permission.update(db=db, obj_in=permission_in, db_obj=permission)
    return permission
