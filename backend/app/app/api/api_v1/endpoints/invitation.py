...
@router.post("/", response_model=List[schemas.Invitation])
def create_invitation(
    *,
    ...
    if crud.user.is_superuser(current_user):
        ...
        raise HTTPException(status_code=400, detail="Not enough permissions")
    return crud.invitation.get_multi(db=db)
...
def update_invitation(
    *,
    ...
    """
    invitation = crud.invitation.get_by_uuid(db=db, uuid=uuid)
    if not invitation:
...
@router.get("/by_uuid/", response_model=schemas.Invitation)
def read_invitation(
    *,
    db: Session = Depends(deps.get_db),
    uuid: str,
...
def delete_invitation(
    *,
    ...
    if invitation.email != current_user.email:
        raise HTTPException(status_code=400, detail="Not enough permissions")
    invitation = crud.invitation.remove_uuid(db=db, uuid=uuid)
    return invitation
