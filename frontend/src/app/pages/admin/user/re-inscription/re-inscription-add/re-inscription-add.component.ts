...
export class ReInscriptionAddComponent implements OnInit {
  ...
  all_semester = ["S1", "S2", "S3", "S4", "S5", "S6", "S7", "S8", "S9", "S10"]

  licence: Semester[] = [{ label: "S1", value: "S1", checked: false, disabled: false }, { label: "S2", value: "S2", checked: false, disabled: false },
  ...
  setSelectedSemester(infSemester: string, supSemester: string) {
    const valueInf = infSemester.substring(1)
    const valueSup = supSemester.substring(1)
    if (valueInf)
      if (+valueInf <= 6) {
        this.licence.forEach((licence: Semester) => {
          if (licence.value === "S" + valueInf) {
            licence.checked = true;
          }
        });
        this.checkLicence()
      } else if (+valueInf <= 8) {
        this.master_1.forEach((master: Semester) => {
          if (master.value === "S" + valueInf) {
            master.checked = true;
          }
        });
        this.checkMasterOne()
      } else {
        this.master_2.forEach((master: Semester) => {
          if (master.value === "S" + valueInf) {
            master.checked = true;
          }
        });
  ...
  async ngOnInit() {
    ...
    if (numCarte && numCarte.length > 0 && year) {
      ...
      this.setSelectedSemester(data.inf_semester, data.sup_semester)
  ...
  async submitForm() {
    if (this.form.valid) {
      ...
      const body =
      {
        ...
        inf_semester: this.infSemester,
        sup_semester: this.supSemester
      }
      console.log(body);
